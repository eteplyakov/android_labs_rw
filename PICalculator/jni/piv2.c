...
void mp_add(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
		fft_int out[]);
void mp_sub(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
		fft_int out[]);
void mp_imul(fft_int n, fft_int radix, fft_int in1[], fft_int in2,
		fft_int out[]);
fft_int mp_idiv(fft_int n, fft_int radix, fft_int in1[], fft_int in2,
		fft_int out[]);
...
fft_float tmpfft[]);
void mp_mul(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
		fft_int out[],
		fft_int tmp[], fft_int nfft, fft_float tmp1fft[], fft_float tmp2fft[],
		fft_float tmp3fft[]);
void mp_squ(fft_int n, fft_int radix, fft_int in[], fft_int out[],
		fft_int tmp[],
		fft_int nfft, fft_float tmp1fft[], fft_float tmp2fft[]);
void mp_mulhf(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
		fft_int out[],
		fft_int tmp[], fft_int nfft, fft_float in1fft[], fft_float tmpfft[]);
void mp_mulhf_use_in1fft(fft_int n, fft_int radix, fft_float in1fft[],
		fft_int in2[],
		fft_int out[], fft_int tmp[], fft_int nfft, fft_float tmpfft[]);
void mp_squhf_use_infft(fft_int n, fft_int radix, fft_float infft[],
		fft_int in[],
		fft_int out[], fft_int tmp[], fft_int nfft, fft_float tmpfft[]);
void mp_mulh(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
		fft_int out[],
...
jstring Java_com_example_picalculator_PiCalculateService_calculatePi(
		JNIEnv *env, jobject obj, jint fft, jstring precision) {
	fft_int nfft, log2_nfft, radix, log10_radix, n, npow, nprc;
	fft_float err;
	...
	jobject g_obj;
	jmethodID g_mid, g_mid2;
	JNIEnv * g_env;

	nfft = (fft_int) fft;
	const char *precisionName = (*env)->GetStringUTFChars(env, precision, 0);

	g_obj = (*env)->NewGlobalRef(env, obj);
	jclass g_clazz = (*env)->GetObjectClass(env, g_obj);
	g_mid = (*env)->GetMethodID(env, g_clazz, "callback", "(I)V");
	g_mid2 = (*env)->GetMethodID(env, g_clazz, "getStopFlag", "()I");
	...
	int progress = step;
	do {
		stopFlag = (int) (*env)->CallBooleanMethod(env, g_obj, g_mid2);
		if (stopFlag == 1) {
			break;
		}
		(*env)->CallVoidMethod(env, g_obj, g_mid, progress);
		progress += step;
	...
	(*env)->CallVoidMethod(env, g_obj, g_mid, 100);
	...
	/* ---- output ---- */
	char out_dir[64] = "";
	sprintf(out_dir, "/sdcard/calculated pi (%s).txt", precisionName);
	f_out = fopen(out_dir, "w+");
	//printf("writing pi.dat...\n");
	mp_fprintf(n - 1, log10_radix, a, f_out);
	fclose(f_out);
	(*env)->ReleaseStringUTFChars(env, precision, precisionName);
	free(d3);
	free(d2);
	free(d1);
	free(i2);
...
void mp_add(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
	...
	if (outsgn >= 0) {
		...
		if (expdif >= 0) {
			outexp += mp_unexp_add(n, radix, expdif, (dgt_int *) &in1[2],
					(dgt_int *) &in2[2], (dgt_int *) &out[2]);
		} else {
			outexp += mp_unexp_add(n, radix, -expdif, (dgt_int *) &in2[2],
					(dgt_int *) &in1[2], (dgt_int *) &out[2]);
		}
	} else {
		outsgn = mp_unsgn_cmp(n, &in1[1], &in2[1]);
		if (outsgn >= 0) {
			expdif = mp_unexp_sub(n, radix, expdif, (dgt_int *) &in1[2],
					(dgt_int *) &in2[2], (dgt_int *) &out[2]);
		} else {
			expdif = mp_unexp_sub(n, radix, -expdif, (dgt_int *) &in2[2],
					(dgt_int *) &in1[2], (dgt_int *) &out[2]);
		}
		outexp -= expdif;
		outsgn *= in1[0];
...
void mp_imul(fft_int n, fft_int radix, fft_int in1[], fft_int in2,
		fft_int out[]) {
	void mp_unsgn_imul(fft_int n, fft_float dradix, fft_int in1[],
			fft_float din2,
			fft_int out[]);
...
fft_int mp_idiv(fft_int n, fft_int radix, fft_int in1[], fft_int in2,
		fft_int out[]) {
	void mp_load_0(fft_int n, fft_int radix, fft_int out[]);
	void mp_unsgn_idiv(fft_int n, fft_float dradix, fft_int in1[],
			fft_float din2,
			fft_int out[]);
...
void mp_mul(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
	...
	fft_float d3[]);
	void mp_mul_d2i(fft_int n, fft_int radix, fft_int nfft, fft_float din[],
			fft_int out[]);
	fft_int n_h, shift;
	dgt_int *in1dgt, *in2dgt;
...
void mp_squh_use_in1fft(fft_int n, fft_int radix, fft_float inoutfft[],
		fft_int out[],
		fft_int nfft) {
	void mp_mul_csqu_nt_d1(fft_int nfft, fft_float d1[]);
	void mp_mul_d2i(fft_int n, fft_int radix, fft_int nfft, fft_float din[],
...
fft_float d3[]) {
	void cdft(fft_int n, fft_int isgn, fft_float *a);
	void mp_mul_rcmul_nt_in1_add(fft_int n, fft_float *a, fft_float *b,
			fft_float *badd);
	fft_float xr, xi;
...
fft_float tmp1fft[], fft_float tmp2fft[]) {
	...
	fft_int mp_inv_newton(fft_int n, fft_int radix, fft_int in[],
			fft_int inout[],
	...
	do {
		...
		prc = mp_inv_newton(n_nwt, radix, in, out, tmp1, tmp2, nfft_nwt,
				tmp1fft, tmp2fft);
...
fft_float tmp1fft[], fft_float tmp2fft[]) {
	void mp_load_0(fft_int n, fft_int radix, fft_int out[]);
	fft_int mp_get_nfft_init(fft_int radix, fft_int nfft_max);
	void mp_sqrt_init(fft_int n, fft_int radix, fft_int in[], fft_int out[],
			fft_int out_rev[]);
	fft_int mp_sqrt_newton(fft_int n, fft_int radix, fft_int in[],
			fft_int inout[],
			fft_int inout_rev[], fft_int tmp[], fft_int nfft,
			fft_float tmp1fft[],
	...
	do {
		...
		prc = mp_sqrt_newton(n_nwt, radix, in, out, tmp1, tmp2, nfft_nwt,
				tmp1fft, tmp2fft, &n_tmp1fft);
		if (thr * nfft_nwt >= nfft) {
			thr = 0;
			if (2 * prc <= n_nwt - 2) {
				nfft_nwt >>= 1;
...
fft_float tmp1fft[], fft_float tmp2fft[]) {
	...
	fft_int mp_invisqrt_newton(fft_int n, fft_int radix, fft_int in,
			fft_int inout[],
			fft_int tmp1[], fft_int tmp2[], fft_int nfft, fft_float tmp1fft[],
			fft_float tmp2fft[]);
	fft_int n_nwt, nfft_nwt, thr, prc;
	...
	do {
		...
		prc = mp_invisqrt_newton(n_nwt, radix, in, out, tmp1, tmp2, nfft_nwt,
				tmp1fft, tmp2fft);
		if (thr * nfft_nwt >= nfft) {
			thr = 0;
			if (2 * prc <= n_nwt - 2) {
...
fft_float tmp2fft[]) {
	...
	void mp_mulh(fft_int n, fft_int radix, fft_int in1[], fft_int in2[],
			...
			fft_int nfft, fft_float in1fft[], fft_float outfft[]);
	void mp_mulh_use_in1fft(fft_int n, fft_int radix, fft_float in1fft[],
	fft_int shift, fft_int in2[], fft_int out[], fft_int nfft,
			fft_float outfft[]);
...
fft_int mp_invisqrt_newton(fft_int n, fft_int radix, fft_int in,
	...
	void mp_imul(fft_int n, fft_int radix, fft_int in1[], fft_int in2,
			fft_int out[]);
	void mp_idiv_2(fft_int n, fft_int radix, fft_int in[], fft_int out[]);
	void mp_squh_save_infft(fft_int n, fft_int radix, fft_int in[],
			fft_int out[],
			fft_int nfft, fft_float infft[], fft_float outfft[]);
	void mp_mulh_use_in1fft(fft_int n, fft_int radix, fft_float in1fft[],
	fft_int shift, fft_int in2[], fft_int out[], fft_int nfft,
